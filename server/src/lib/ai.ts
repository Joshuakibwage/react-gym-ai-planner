...
export async function generateTrainingPlan( 
    ...
    if( !apiKey ) {
        throw new Error("OPEN_ROUTER_KEY is not set in environmental variables.");
...
function formatPlanResponse( 
    aiResponse: unknown, profile: UserProfile 
    ...
    const plan: TrainingPlan = {
        overview: {
            ...
            notes: aiResponse.overview?.notes || "Follow the program consistently for optimal results.",
        },
        weeklySchedule: (aiResponse.weeklySchedule || []).map((day: unknown) => ({
            day: day.day || "Day",
            focus: day.focus || "Full Body",
            exercises: (day.exercises || []).map((exercise: unknown) => ({
        ...
        progression: aiResponse.progression || "Increase weight by 2.5-5lbs when you can complete all sets with good form. Track your progress weekly.",
...
function buildPrompt( profile: UserProfile ): string {
    const goalMap: Record<string, string> = {
        ...
        cut: "loose fat and maintain muscle",
    ...
    const equipmentMap: Record<string, string> = {
        ...
        home_gym: "home gym access to limited equipment",
    ...
    return `create a personalized ${profile.days_per_week}-day per week training plan for someone wih the following profile: 
